...
weighting = np.array([[500, -50, 10, 5, 5, 10, -50, 500],
                      [-50, -90, 1, 1, 1, 1, -90, -50],
                      [10, 1, 3, 2, 2, 3, 1, 10],
                      [5, 1, 2, 1, 1, 2, 1, 5],
                      [5, 1, 2, 1, 1, 2, 1, 5],
...
weights_am = np.array([[20, -3, 11, 8, 8, 11, -3, 20],
                       [-3, -7, -4, 1, 1, -4, -7, -3],
                       [11, -4, 2, 2, 2, 2, -4, 11],
                       [8, 1, 2, -3, -3, 2, 1, 8],
                       [8, 1, 2, -3, -3, 2, 1, 8],
...
class AI(object):
    ...
    def alpha_beta_pruning(self, color, chessboard, alpha, beta, tree_depth):
        ...
        if tree_depth == 0 or time.time() - self.start_time > self.time_out - 0.3:
    ...
    def calculate_value(self, chessboard, len_available_move, color):
        ...
        if self.cal_num(chessboard) < 20:
            return value + (len_available_move - len_reverse_move) * 20 + self.eval_potential(chessboard, color) * 20
        else:

            return value + (len_available_move - len_reverse_move) * 35 \
                   + self.eval_potential(chessboard, color) * 25  # + self.eval_stable(chessboard, color) * 200
                        ...
                        if 8 > row + dr[i] >= 0 and 8 > col + dc[i] >= 0:
                            ...
                            if chessboard[r][c] == -color:
                                if self.next_search(r, c, i, chessboard, row, col, color, available_move):
                                    break
        available_move.sort(key=lambda x: weights_am[x[0]][x[1]], reverse=True)
